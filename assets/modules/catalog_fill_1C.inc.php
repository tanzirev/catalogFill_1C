 ...
 * @author-update Tanzirev <tanzirev@mail.ru>
 */

if(!isset($start1Csync)){defined('IN_MANAGER_MODE') or die();}
...
if(!isset($start1Csync)){define("MODULE_PATH","../assets/modules/catalogFill_1C/");}else{define("MODULE_PATH",dirname(__FILE__).'/');}
...
if(isset($start1Csync)){$config_file=MODULE_PATH.'config/'.$settings['file_settings'].'.php';}
require $config_file;
...
if(isset($start1Csync)){$cf_action = "import";}

switch ($cf_action){
    case "save_settings_1C_sync":
    if(empty($_POST['link_settings']) || empty($_POST['file_settings']) || empty($_POST['parent_id_settings'])){
        $catalogFill->message = "Ошибка при сохранении (не указан один из параметров)"; false;
    }else{
    ...
    case "import":
        ...
        if(isset($start1Csync)){
            $parent_id=$settings['parent_id_settings'];
            $file_name = $cf_config['import_file_name'];
        }
        if(strtolower(substr($file_name,-3))=='csv'){
            $catalogFill->csv_import($parent_id,$file_name,$start1Csync);
